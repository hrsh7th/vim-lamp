...
function! s:Tooltip.new(option) abort
  ...
  call setbufvar(l:buffer, '&filetype', 'markdown')
...
function! s:Tooltip.show(bufpos, contents) abort
  ...
  call nvim_buf_set_lines(self.buffer, 0, -1, v:true, l:lines)

  if !self.is_showing()
    let self.window = nvim_open_win(self.buffer, v:false, self.get_config())
