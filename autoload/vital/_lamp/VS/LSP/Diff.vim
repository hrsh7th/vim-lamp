...
execute join(['function! vital#_lamp#VS#LSP#Diff#import() abort', printf("return map({'try_enable_lua': '', 'compute': ''}, \"vital#_lamp#function('<SNR>%s_' . v:key)\")", s:_SID()), 'endfunction'], "\n")
delfunction s:_SID
" ___vital___
"
" compute
...
function! s:compute(old, new) abort
  ...
  if s:is_lua_enabled
    let l:first_line = luaeval('vital_vs_lsp_diff_search_first_line(_A[1], _A[2])', [l:old, l:new])
  else
    let l:first_line = 0
  ...
  if s:is_lua_enabled
    let l:last_line = luaeval('vital_vs_lsp_diff_search_last_line(_A[1], _A[2], _A[3])', [l:old, l:new, l:first_line])
  else
    let l:last_line = -1
...
lua <<EOF
function vital_vs_lsp_diff_search_first_line(old, new)
...
function vital_vs_lsp_diff_search_last_line(old, new, first_line)
  local old_len = #old
  local new_len = #new
  local min_len = math.min(#old, #new)
  local last_line = -1
